//===- Passes.h - SODA-OPT sparse tensor pipeline entry points -----------*- C++ -*-===//
//
...
//   llvm-project/mlir/include/Dialect/SparseTensor/Pipelines/Passes.h
// Implements all the original sparse-compiler options with a few additions.
//
//===----------------------------------------------------------------------===//
//
...
#ifndef SODA_SPARSETENSOR_PIPELINES_PASSES_H
#define SODA_SPARSETENSOR_PIPELINES_PASSES_H
...
/// Options for the "soda-sparse-compiler" pipeline.  So far this only contains
...
/// for those passes.
struct SODASparseCompilerOptions
    : public PassPipelineOptions<SODASparseCompilerOptions> {
  ...
  PassOptions::Option<bool> enableMemrefPrints{
      *this, "enable-st-prints",
      desc("Enable sparse tensor debugging via memref printing"), init(false)};
...
/// Adds the "soda-sparse-compiler" pipeline to the `OpPassManager`.  This
...
void buildSODASparseCompiler(OpPassManager &pm,
                         const SODASparseCompilerOptions &options);

/// Registers all pipelines for the `sparse_tensor` dialect.  At present,
/// this includes only "soda-sparse-compiler".
void registerSODASparseTensorPipelines();

} // namespace soda
} // namespace mlir

#endif // SODA_SPARSETENSOR_PIPELINES_PASSES_H
